  ...
  function shunpayLoginController($log, $rootScope, httpRequest, permission, $state, alertify, jQ, loc_language_cn, loc_language_en, transfer, getDefault, $timeout, finditem, $scope, heartBeat) {
    ...
    vm.result = function (data) {
      if (data.code == 200) {
        ...
        $state.go('shunpayhome.shunpayhomerecord');
    ...
    vm.getImg();
    vm.setPlatfrom = function (data) {
      console.log("data:" + data);
      // for(var i =  0;i< data.length;i++){
      $rootScope.moreplatfrom = data.data;
      // }
    }
    vm.getPlatfrom = function () {
      httpRequest.get("agent?name=&type&create=&agentName="+window.localStorage.name+"&state=&page=1&size=500", vm.setPlatfrom)
    }
    vm.login = function () {
      if (vm.secured != '0')

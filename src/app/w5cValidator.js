...
angular.module("w5c.validator", ["ng"])
  .provider('w5cValidator', [function () {
    ...
    //  '两次输入不一致': 'two inputs did not match',
    //  '该选项输入格式不正确': 'wrong format input',
    //  '必须输入数字': 'must be in numbers',
    //  '该输入值已经存在，请重新输入': 'value already exists, pls try again',
    var lagchange = {
        "必填": "field",
        "该选项输入值长度不能大于{maxlength}": "该选项输入值长度不能大于{maxlength}",
        "该选项输入值长度不能小于{minlength}": "该选项输入值长度不能小于{minlength}",
        "输入不一致": "two inputs did not match",
        "格式不正确": "wrong format input",
        "必须数字": "must be in numbers",
        "该输入值已经存在，请重新输入": "value already exists, pls try again",
        "输入URL格式不正确": "输入URL格式不正确",
        "该选项输入值不能大于{max}": "该选项输入值不能大于{max}",
        "该选项输入值不能小于{min}": "该选项输入值不能小于{min}"
      ...
      this.defaultShowError = function (elem, errorMessages) {
        ...
        if (!this.isEmpty($group) && !$group.hasClass("has-error")) {
          if (typeof(errorMessages[0]) != 'undefined') {
            ...
            //else
            //  $elem.parent().after('<span class="w5c-error">' + errorMessages[0] + '</span>');
          }
        }
      };
      ...
      showError: function (elem, errorMessages, options) {
        ...
        for (var i = 0; i < errorMessages.length; i++) {
          if (localStorage["language"] == 'en') {
            errorMessages[i] = lagchange[errorMessages[i]];
          }
        }
...
angular.module("w5c.validator")
  .directive("w5cFormValidate", ['$parse', 'w5cValidator', '$timeout', '$rootScope', function ($parse, w5cValidator, $timeout, $rootScope) {
    return {
              ...
              $elem.bind("blur", function () {
                ...
                $timeout(function () {
                  if (!scope[formName][self.name].$valid) {
                    ...
                    //w5cValidator.showError($elem, $rootScope.lagconfig[errorMessages], options);
                    for (var i = 0; i < errorMessages.length; i++) {
                      errorMessages[i] = $rootScope.lagconfig[errorMessages[i]];
                    }

...
function createLinkstring(para) {
    var arg = "";
    for (i in para) {
        if ((!para[i] && typeof para[i] != 'number') || i == 'password' || (i + '').toLowerCase() == 'creattime')
            continue; //跳过传递此类
...
function qrcode_timeout() { //二维码超时则停止显示二维码
...
function show_Qrcode(data) {
    ...
    var oldMoney = data.money != data.price ? '<del style="font-size:22px;">￥' + data.price + '</del>' : '';
    if (data.qrcode)$("#show_qrcode").attr("src", data.qrcode); //二维码更新
    if (data.money) $("#money").html('￥' + data.money + oldMoney); //金额改变 云端匹配最佳金额
...
show_desc = function (data) { //商品描述
    ...
    html += getDescMode('支付单号', data.order_id);
    html += getDescMode('创建时间', myDate("y-m-d h:m:s", data.createTIme));
